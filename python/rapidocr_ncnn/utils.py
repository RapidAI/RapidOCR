...
class NCNNInferSession:
    def __init__(self, config):
        self.session = ncnn.Net()

        model_dir = Path(config["model_path"])
        param_path = model_dir / f"{model_dir.name}.param"
        bin_path = model_dir / f"{model_dir.name}.bin"

        self._verify_model(param_path)
        self._verify_model(bin_path)

        self.session.load_param(str(param_path))
        self.session.load_model(str(bin_path))

    def __call__(self, input_content: np.ndarray) -> np.ndarray:
        input_blob = ncnn.Mat.from_pixels(
            input_content,
            ncnn.Mat.PixelType.PIXEL_BGR,
            input_content.shape[1],
            input_content.shape[0],
        )

        ex = self.session.create_extractor()
        input_names = self.get_input_names()
        output_names = self.get_output_names()

        ex.input(input_names[0], input_blob)

        output_blob = ncnn.Mat()
        ex.extract(output_names[0], output_blob)

        output_data = np.array(output_blob)
        return output_data

    def get_input_names(
        self,
    ):
        return [v for v in self.session.input_names()]

    def get_output_names(
        self,
    ):
        return [v for v in self.session.output_names()]
