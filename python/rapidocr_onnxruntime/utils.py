...
class UpdateParameters:
    ...
    def __call__(self, config, **kwargs):
        ...
        new_config = {
            ...
            "Det": self.update_model_params(config["Det"], det_dict, "det_"),
            "Cls": self.update_model_params(config["Cls"], cls_dict, "cls_", ["cls_label_list", "cls_model_path", "cls_use_cuda"]),
            "Rec": self.update_model_params(config["Rec"], rec_dict, "rec_", ["rec_model_path", "rec_use_cuda"]),
    ...
    def update_model_params(self, config, model_dict, prefix: str, need_remove_prefix: List[str] = None):
        if not model_dict:
            return config

        new_model_dict = self.remove_prefix(model_dict, prefix, need_remove_prefix)
        config.update(new_model_dict)
        return config
        

    @staticmethod
    def remove_prefix(
        config: Dict[str, str], prefix: str, remove_params: List[str] = None
    ) -> Dict[str, str]:
        new_rec_dict = {}
