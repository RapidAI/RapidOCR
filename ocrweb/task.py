...
def detect_recognize(image_path, is_api=False, security_check = False):
    ...
    return img_str, elapse, elapse_part, rec_res_data, security_check
...
def detection(rec_res_data, js=True, css=True, url=True, il_word=True):
    ...
    if js:
        det_res = det.js_test()
    if css:
        det_res = det.css_test()
    if url:
        det_res = det.url_test()
    if il_word:
        det_res = det.il_word_test()
    return det_res